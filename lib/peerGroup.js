...
var DEFAULT_PXP_PORT = 8192 // default port for peer-exchange servers
...
class PeerGroup extends EventEmitter {
  ...
  _connectPeer (cb) {
    ...
    if (this._exchange.peers.length > 0) {
  ...
  _connectWebSeeds (cb) {
    ...
    async.each(this._params.webSeeds, (peer, cb) => {
      ...
      this._exchange.connect(peer.transport, peer.address, peer.opts,
        (err, peer) => {
          this._onNewConnection(err, peer)
          cb()
        })
    }, cb)
  }
  ...
  _fillPeers () {
    ...
    for (var i = 0; i < n; i++) {
      this._connectPeer(this._onNewConnection.bind(this))
    }
  }
